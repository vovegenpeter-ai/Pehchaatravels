import type { Tour, Hotel, Destination, Testimonial, TourImage, HotelImage, Category } from '@prisma/client'

export type TourWithRelations = Tour & {
  images?: TourImage[]
  category?: Category | null
  faqs?: unknown
}

export type HotelWithRelations = Hotel & {
  images?: HotelImage[]
  category?: Category | null
}

export function mapTour(tour: TourWithRelations) {
  return {
    id: tour.id,
    slug: tour.slug,
    name: tour.name,
    destination: tour.destination,
    location: tour.location,
    description: tour.shortDescription,
    shortDescription: tour.shortDescription,
    fullDescription: tour.fullDescription,
    price: Number(tour.price),
    rating: tour.rating,
    image: tour.bannerImage,
    bannerImage: tour.bannerImage,
    bannerImagePublicId: tour.bannerImagePublicId ?? null,
    images: tour.images?.sort((a, b) => a.order - b.order).map((i) => i.url) ?? [],
    imagesMeta: tour.images?.sort((a, b) => a.order - b.order).map((i) => ({ url: i.url, publicId: i.publicId ?? null })) ?? [],
    startDate: tour.startDate,
    startTime: tour.startTime,
    endDate: tour.endDate,
    endTime: tour.endTime,
    meetingPoint: tour.meetingPoint,
    itinerary: (tour.itinerary as Array<Record<string, unknown>>) ?? [],
    faqs: (tour.faqs as Array<Record<string, unknown>>) ?? [],
    includedServices: tour.includedServices,
    excludedServices: tour.excludedServices,
    maxGuests: tour.maxGuests,
    published: tour.published,
    featured: tour.featured,
    latest: tour.latest,
    categoryId: tour.categoryId,
    category: tour.category,
  }
}

export function mapHotel(hotel: HotelWithRelations) {
  return {
    id: hotel.id,
    slug: hotel.slug,
    name: hotel.name,
    description: hotel.shortDescription || hotel.description,
    shortDescription: hotel.shortDescription || hotel.description,
    fullDescription: hotel.fullDescription || hotel.description,
    location: hotel.location,
    address: hotel.address,
    pricePerNight: Number(hotel.pricePerNight),
    rating: hotel.rating,
    contactPhone: hotel.contactPhone,
    contactEmail: hotel.contactEmail,
    checkInTime: hotel.checkInTime,
    checkOutTime: hotel.checkOutTime,
    amenities: hotel.amenities,
    roomTypes: hotel.roomTypes,
    image: hotel.bannerImage,
    bannerImage: hotel.bannerImage,
    bannerImagePublicId: hotel.bannerImagePublicId ?? null,
    images: hotel.images?.sort((a, b) => a.order - b.order).map((i) => i.url) ?? [],
    imagesMeta: hotel.images?.sort((a, b) => a.order - b.order).map((i) => ({ url: i.url, publicId: i.publicId ?? null })) ?? [],
    published: hotel.published,
    featured: hotel.featured,
    categoryId: hotel.categoryId,
    category: hotel.category,
  }
}

export function mapDestination(dest: Destination) {
  return {
    id: dest.id,
    slug: dest.slug,
    name: dest.name,
    description: dest.shortDescription || dest.description,
    shortDescription: dest.shortDescription || dest.description,
    fullDescription: dest.fullDescription || dest.description,
    location: dest.location,
    image: dest.image,
    imagePublicId: dest.imagePublicId ?? null,
    published: dest.published,
    featured: dest.featured,
    orderNumber: dest.orderNumber ?? 0,
    categoryId: dest.categoryId,
  }
}

export function mapTestimonial(t: Testimonial) {
  return {
    id: t.id,
    name: t.name,
    text: t.text,
    rating: t.rating,
    avatar: t.avatar,
    published: t.published,
    featured: t.featured,
  }
}
