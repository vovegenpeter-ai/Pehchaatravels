...
export default function AdminDestinationsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (c.status === 'fulfilled') setCategories(c.value.filter((cat) => cat.type === 'DESTINATION'))
      } finally {
  ...
  /* Group destinations: Category → Location → Places */
  const tree = useMemo(() => {
    ...
    filtered.forEach((d) => {
      ...
      if (!catMap[catId]) catMap[catId] = { id: catId, name: d.category?.name || 'Uncategorized', locations: {} }
      const loc = d.location || 'Other'
      if (!catMap[catId].locations[loc]) catMap[catId].locations[loc] = []
      catMap[catId].locations[loc].push(d)
    })
    return Object.values(catMap)
  }, [destinations, search, filterCategory])
      ...
      <div className="places-tree">
        {loading ? (
          ...
          tree.map((cat) => {
            ...
            const locationKeys = Object.keys(cat.locations)
            return (
              <div key={cat.id} className="tree-category">
                ...
                <div className="tree-category__header" onClick={() => toggleCategory(cat.id)}>
                  ...
                  <span className="tree-category__count">{Object.values(cat.locations).flat().length} places</span>
                </div>
                ...
                {isExpanded && (
                  <div className="tree-category__children">
                    {locationKeys.map((loc) => {
                      const locKey = `${cat.id}-${loc}`
                      const locExpanded = expandedLocations[locKey] !== false
                      const places = cat.locations[loc]
                      return (
                        <div key={loc} className="tree-location">
                          <div className="tree-location__header" onClick={() => toggleLocation(locKey)}>
                            ...
                            <span className="tree-location__name">{loc}</span>
                            <span className="tree-location__count">{places.length}</span>
                          </div>
